const $ = id => document.getElementById(id);
    ...
    async function loadPage(page, force) {
      if (state.activePageModule?.unmount) {
        await Promise.resolve(state.activePageModule.unmount(pageContext(false))).catch(handleError);
      }
    ...
    document.addEventListener("click", async event => {
      ...
      try {
        ...
        else if (action === "create-subscriber") openCreateSubscriber();
        ...
        } else if (action === "refresh-config-detail") {
          await renderConfigDetail(id);
          notify("配置已刷新", false);
        } else if (action === "restart-config-plugin") {
          await restartCurrentConfigPlugin();
        } else if (action === "save-config") {
          await saveCurrentConfig();
        } else if (action === "toggle-config-secret") {
          await toggleConfigSecret(button);
        } else if (action === "add-command-permission") {
          await openCommandPermissionModal();
        } else if (action === "delete-command-permission") {
          const rules = commandPermissionRules();
          rules.splice(Number(button.dataset.index), 1);
          setCommandPermissionRules(rules);
        } else if (action === "clear-command-permissions") {
          if (!confirm("确定清空所有权限规则吗？")) return;
