...
public object DynamicFilterRuleTable : IntIdTable("dynamic_filter_rule") {
    ...
    public val action: Column<FilterAction> = enumerationByName<FilterAction>("action", 20)
    public val condition: Column<FilterCondition> = registerColumn("condition_json", filterConditionColumn())
    public val priority: Column<Int> = integer(name = "priority").default(0)
    public val enabled: Column<Boolean> = bool(name = "enabled").default(true)
